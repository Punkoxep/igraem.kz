...
export class GroundsController {
  ...
  public static async getAllGrounds(req: Request, res: Response) {
    try {
      ...
      const formattedGrounds = grounds.map((g: any) => {
        const totalReviews = g.reviews ? g.reviews.length : 0;
        const avgRating = totalReviews > 0
          ? Number((g.reviews.reduce((acc: number, r: any) => acc + r.rating, 0) / totalReviews).toFixed(1))
          : 0.0;
        ...
        return {
          ...
          occupied_slots: g.bookings || [],
        };
      });

      return res.json({
        success: true,
        data: formattedGrounds,
      });
    } catch (error: any) {
  ...
  /**
   * Submit review & star rating for a ground (1 to 5 stars)
   */
  public static async addReview(req: Request, res: Response) {
    try {
      ...
      if (!rating || rating < 1 || rating > 5) {
        return res.status(400).json({ success: false, message: 'Укажите рейтинг от 1 до 5 звезд' });
      }

      await prisma.review.create({
        data: {
          ground_id: id,
          user_id: userId || 'seed-admin-user',
          rating: parseInt(rating),
          comment: comment || '',
        },
      });

      const reviews = await prisma.review.findMany({ where: { ground_id: id } });
      const total = reviews.length;
      const newAvg = total > 0 ? Number((reviews.reduce((acc, r) => acc + r.rating, 0) / total).toFixed(1)) : 0.0;
      ...
      return res.json({
        success: true,
        message: 'Отзыв успешно сохранен и рейтинг пересчитан',
        rating: newAvg,
        reviews_count: total,
      });
    } catch (error: any) {
